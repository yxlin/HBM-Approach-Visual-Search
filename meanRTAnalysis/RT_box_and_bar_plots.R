...
# Subset to present trials and absent trials -------------------
featP <- subset(avgRT1, task == "F" & target == "P")
conjP <- subset(avgRT1, task == "C" & target == "P")
spatP <- subset(avgRT2, task == "S" & target == "P")

featA <- subset(avgRT1, task == "F" & target == "A")
conjA <- subset(avgRT1, task == "C" & target == "A")
spatA <- subset(avgRT2, task == "S" & target == "A")
...
# Box plots--------------------------------
boxRT.Err.Absent <- subset(boxRT.Err, target=="A")
...
# jpeg(filename = "./figures/boxplots_and_barplots_RT_absentTrial.jpeg",
#      width = 1280, height = 1024, units = "px", pointsize = 8,
#      quality = 95 ,bg = "white")
multiplot(boxrt, barRT, boxErr, barErr, cols=2); 
# dev.off()
