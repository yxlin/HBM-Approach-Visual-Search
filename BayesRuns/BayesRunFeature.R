...
for(i in seq(along = size.seq)){
  ...
  for(j in seq(along = target.seq)){
    ...
    # theta (scale) = lambda^(-1/beta); psi = min(rt.sec) - .2
...
# Test run ---------------------------------------------------------
# jagsfitT <- jags(data=JAGSName, model.file='./BayesRuns/model.txt', inits=initList, parameters.to.save=parameters, n.iter=nit, n.chain = nct, n.burnin = nbt, n.thin = ntt)
